...
function uniq(arr, cb) {
  let uniqueArr = [];
  //Essentially a for loop to see what elements are in the array, if the unique array doesn't already include that value, add it
  arr.forEach((notRepeat) => {
    if (!uniqueArr.includes(notRepeat)) {
      uniqueArr.push(notRepeat);
    }
  });
  return cb(uniqueArr);
}
